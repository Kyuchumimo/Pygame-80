...
def keyp(*args):
    ...
    if len(args) == 0:
        return any(pygame.key.get_just_pressed()[i] for i in keycodes)
    elif len(args) == 1:
        try:
            code = int(args[0])
...
def pmem(index, val32):
    ...
    if val32 == None:
        try:
            with open(f"{_SAVEID}.sav", 'r') as file:
                data = json.load(file)
        ...
        try:
            ...
            with open(f"{_SAVEID}.sav", 'w') as file:
                json.dump(data,file)
